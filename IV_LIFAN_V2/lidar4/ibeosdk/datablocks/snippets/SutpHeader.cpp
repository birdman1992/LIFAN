...
std::istream& SutpHeader::read(std::istream& is)
{
	if (!is || (!is.good())) {
		return is; // return immediately if input stream is in error state
	}
...
std::ostream& SutpHeader::write(std::ostream& os) const
{
	if (!os  ||  (!os.good()))
	{
		return os;
