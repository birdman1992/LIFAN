...
//    unsigned char HWA_CheckSum_201;//校验和,Checksum  = (Byte1  +Byte2  …+  Byte7)  XOR 0xFF
    ...
    float EPS_SteeringWheelAngle;//方向盘角度,顺时针方向为“-”，逆时针方向为“+”
    int EPS_SteeringWheelAngularSpeed;//方向盘角速度,顺时针方向为“-”，逆时针方向为“+”
    float EPS_SteeringTorque;//方向盘角扭矩,顺时针方向为“-”，逆时针方向为“+”
    unsigned char EPS_RollingCounter_221;//计数器
    unsigned char EPS_Checksum_221;//校验和,Checksum  = (Byte1  +Byte2  …+  Byte7)  XOR 0xFF

    void controlAngle(unsigned char ADAS_EN,unsigned char Threshold,float DesireValue,int LimitAngleSpeed);
    void controlTorque(unsigned char Threshold,float DesireValue,int LimitAngleSpeed);
    void validData(unsigned char *data);
};
